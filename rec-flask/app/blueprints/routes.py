...
from app.service.user.user_profile import user_behavior_update, get_user_profile_detail, \
    ensure_user_profile
...
def create_app():
    ...
    def _apply_ranking_pipeline(user_id, recall_num=2000, coarse_num=800, fine_num=400, re_num=200):
        ...
        coarse_df = coarse_result.head(coarse_num) if isinstance(coarse_result, pd.DataFrame) and len(
            coarse_result) > coarse_num else coarse_result
        ...
        fine_df = fine_result.head(fine_num) if isinstance(fine_result, pd.DataFrame) and len(
            fine_result) > fine_num else fine_result
        ...
        re_df = re_result.head(re_num) if isinstance(re_result, pd.DataFrame) and len(re_result) > re_num else re_result
    ...
    def _add_scores_to_products(recommended_products, merged):
        ...
        if merged is not None and 'asin' in merged.columns:
            ...
            score_map = merged_unique.set_index('asin')[
                ['recall_score', 'coarse_score', 'fine_score', 're_score', 'total_score']].to_dict(orient='index')
    ...
    def get_recommendations():
        ...
        return jsonify({
            ...
            'current_page': page,
            'per_page': page_size,
            'products': random.sample(recommended_products, len(recommended_products)) if recommended_products else []
        })
